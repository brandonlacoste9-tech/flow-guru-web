...
// ─── Types for search provider responses ───────────────────────────────────
...
async function executeWebSearchAction(plan: AssistantActionPlan): Promise<AssistantActionResult> {
  ...
  if (!query) {
    return {
      ...
      summary: "What would you like me to search for?",
    };
  }

  const braveKey = process.env.BRAVE_SEARCH_API_KEY;

  // ── Brave Search (preferred, requires BRAVE_SEARCH_API_KEY) ──────────────
  if (braveKey) {
    ...
    const results: WebSearchResult[] = (payload.web?.results ?? []).map(r => ({
      title: r.title,
      url: r.url,
    ...
    if (!results.length) {
      return {
        ...
        summary: `I searched for "${query}" but didn't get any useful results.`,
  ...
  // Instant answer (e.g. calculator, conversions, quick facts)
  if (ddg.Answer) {
    results.push({ title: "Instant answer", url: "", snippet: ddg.Answer });
  }

  // Abstract (Wikipedia/Wikidata summary)
  if (ddg.AbstractText) {
    results.push({
      title: ddg.AbstractSource ?? "Summary",
      url: ddg.AbstractURL ?? "",
      snippet: ddg.AbstractText.slice(0, 300),
    });
  }
  ...
  if (!results.length) {
    return {
      ...
      status: "failed",
      title: "No results found",
      summary: `I searched for "${query}" but DuckDuckGo didn't return useful results. Add a BRAVE_SEARCH_API_KEY for richer web search.`,
      provider: "duckduckgo",
    };
  }
