  ...
  assistant: router({
    ...
    send: rateLimitedProcedure
      ...
      const systemPrompt = [
        ...
        "- If a TOOL RESULT system message is present, your reply MUST reflect that result (success, missing info, or need to connect).",
        "",
        `Language: reply entirely in ${input.language === "fr" ? "French" : "English"}.`,
        "",
        "You can act on (never advertise these unprompted): calendar, weather, directions, contacts, reminders, music, lists, news, web/knowledge when tools run.",
        "",
        "User memory:",
        memoryContext,
      ].join("\n");
